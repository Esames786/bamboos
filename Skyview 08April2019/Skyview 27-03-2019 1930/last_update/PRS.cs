        ...
        private void Generate()
        {
            try
            {
                ...
                if (dv.Rows.Count > 0)
                {
                    ...
                    rt.SetDataSource(dv); viewer_of_table_close aop = new viewer_of_table_close(rt, txt_rec, txt_return, Convert.ToDecimal(txtdis.Text),service_c,Convert.ToDecimal(txt_strn.Text)); aop.Show();
        ...
        private void txtsale_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                DataTable h30 = clsDataLayer.RetreiveQuery(h3); if (h30.Rows.Count > 0)
                {
                    ...
                    gs0 = Convert.ToDecimal(h30.Rows[0][0].ToString());
                    txtbill.Text = gs0.ToString();
