...
namespace Skyview
{
    public partial class Payment_voucher : Form
    {
        ...
        public Payment_voucher()
        {
            ...
            try
            {
                clsGeneral.SetAutoCompleteTextBox(txtparty, clsDataLayer.RetreiveQuery("select ActTitle from Accounts where ID > 14 and Status = 1 and [HeaderActCode] !='10101' Order BY ID DESC"));
        ...
        private void OnNew()
        {
            ...
            try
            {
                ...
                cmbStatus.Text = "Pending"; txtAvailable.Enabled = false; txtparty.Enabled = true;
            }
            catch { }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else { MessageBox.Show("Fill All Fields", "Stop", MessageBoxButtons.OK, MessageBoxIcon.Stop); }
            }
            catch { }
        }
        ...
        private void save()
        {
            try
            {
                ...
                //    else
                //    {
                //        String s2l = "select RemainingAmount,PaidAmount from NPurchase where PurCode='" + txtpo.Text + "'";
                //        DataTable fetchRecord = clsDataLayer.RetreiveQuery(s2l);
                //        if (fetchRecord.Rows.Count > 0)
                //        {
                //            decimal remain = Convert.ToDecimal(fetchRecord.Rows[0][0].ToString());
                //            decimal paid = Convert.ToDecimal(fetchRecord.Rows[0][1].ToString()); decimal remainrel = Convert.ToDecimal(txtCashRelease.Text);
                //            decimal finalrel = remain - remainrel; decimal fpaid = paid + remainrel;
                //            String upd1 = "update NPurchase set RemainingAmount=" + finalrel + ",PaidAmount=" + fpaid + " where PurCode='" + txtpo.Text + "'"; clsDataLayer.ExecuteQuery(upd1);
                //        } 
                //    }
                //
                #endregion comment
        ...
        private void CashTransUpdate()
        {
            ...
            String q2 = "select CashIn,CashCode,Status,Cash,CurrentCash from CashTransaction where CashIn >= " + SID + " Order by CashIn asc"; DataTable d2 = clsDataLayer.RetreiveQuery(q2); if (d2.Rows.Count > 0)
            {
                ...
                string query1 = "";
                query1 = "UPDATE tech_cash SET CASH = '" + oldb + "' where CompanyName = '" + txt_company.Text + "' ";
        ...
        public void search()
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    //  txtpo.Text = dt.Rows[0]["PInvoice"].ToString();
        ...
        private void txtpo_TextChanged(object sender, EventArgs e)
        {
            ...
            //    else
            //    {
            //        String sl2 = "select RemainingAmount from NPurchase where PurCode='" + txtpo.Text + "'"; DataTable d90 = clsDataLayer.RetreiveQuery(sl2);
            //        if (d90.Rows.Count > 0)
            //        {
            //            decimal remain = Convert.ToDecimal(d90.Rows[0][0].ToString()); txtCashRelease.Text = remain.ToString();
            //        }
            //        else {
        ...
        {
            Master_Form ms = new Master_Form();
        }
