...
namespace Skyview
{
    public partial class InvRawProduct : Form
    {
        String btnStatus = "";
        TextBox tb = new TextBox();
        public InvRawProduct()
        {
            InitializeComponent();
            refresh();


            String h3 = "select RCode from tbl_Rawprod";
        ...
        private void New_Click(object sender, EventArgs e)
        {
            ...
            grid.Rows.Add();
            grid.Enabled = true;
            txtcode.Text = clsGeneral.getMAXCode("tbl_Rawprod", "RCode", "RW"); txt_search.Text = "-"; btnStatus = "new";
        }
        ...
        private void grid_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (grid.CurrentCell.ColumnIndex == 0 || grid.CurrentCell.ColumnIndex == 1 || grid.CurrentCell.ColumnIndex == 2)
                {
        ...
        private void Save()
        {
            if (!CheckAllFields() && !CheckDataGridCells(grid))
            {
                ...
                {
                    String sel = "select * from tbl_Rawprod where ProductName='" + grid.Rows[a].Cells[0].Value + "'";
                    DataTable gb = clsDataLayer.RetreiveQuery(sel);
                    if (gb.Rows.Count > 0)
                    {
                        MessageBox.Show("Already Add!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    }
                    else
                    {
                        String ins = "insert into tbl_Rawprod(RCode,ProductName,QuantityType,PurchasPrice,Dates,UserName)values('" + txtcode.Text + "','" + grid.Rows[a].Cells[0].Value.ToString() + "','" + grid.Rows[a].Cells[1].Value.ToString() + "'," + grid.Rows[a].Cells[2].Value.ToString() + ",'" + DateTime.Now.ToString("yyyy-MM-dd") + "','" + Login.UserID + "')";
        ...
        private void btnsave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Fill All Fields!", "Stop", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                }
            }
            catch { }
        }
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (!CheckAllFields() && !CheckDataGridCells(grid))
            {
                for (int a = 0; a < grid.Rows.Count; a++)
                {
                    String sel = "select * from tbl_Rawprod where ProductName='" + grid.Rows[a].Cells[0].Value + "' and RCode != '"+txtcode.Text+"'";
                    DataTable gb = clsDataLayer.RetreiveQuery(sel);
                    if (gb.Rows.Count > 0)
                    {
                        MessageBox.Show("Already Add!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                ...
                if (m == false)
                {
                    ...
                    String del = "delete from tbl_Rawprod where RCode='" + txtcode.Text + "'"; clsDataLayer.ExecuteQuery(del); Save();
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                SqlDataAdapter sda = new SqlDataAdapter("SELECT distinct(RCode) FROM tbl_Rawprod Where RCode = '" + txt_search.Text + "'   ", con);
        ...
        public void refresh()
        {
            SqlDataAdapter sda = new SqlDataAdapter("SELECT distinct(RCode) FROM tbl_Rawprod", con);
            DataTable dt = new DataTable();
            sda.Fill(dt);
            if (dt.Rows.Count > 0)
            {
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            SqlDataAdapter sda2 = new SqlDataAdapter("SELECT distinct(RCode) FROM tbl_Rawprod Where RCode ='" + txt_search.Text + "'", con);
            ...
            SqlDataAdapter sda = new SqlDataAdapter("SELECT RCode,ProductName,QuantityType,PurchasPrice,Dates,UserName FROM tbl_Rawprod Where RCode ='" + txt_search.Text + "'", con);
            ...
            if (dt1.Rows.Count > 0)
            {
                ...
                foreach (DataRow row in dt1.Rows)
                {
                    ...
                    grid.Rows[n].Cells[1].Value = row[2].ToString(); grid.Rows[n].Cells[2].Value = row[3].ToString();

                }
                String gb = "SELECT distinct(RCode) FROM tbl_Rawprod";
        ...
        private void grid_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (!btnStatus.Equals("edit"))
                {
                    if (grid.CurrentCell.ColumnIndex == 0)
                    {
                        String sel = "select * from tbl_Rawprod where ProductName='" + grid.CurrentRow.Cells[0].Value.ToString() + "'";
                        DataTable gb = clsDataLayer.RetreiveQuery(sel);
                        if (gb.Rows.Count > 0)
                        {
                            MessageBox.Show("Already Add!", "Stop", MessageBoxButtons.OK, MessageBoxIcon.Stop); grid.CurrentRow.Cells[0].Value = "";
                        }
        ...
        private void btnprint_Click(object sender, EventArgs e)
        {
            try
            {
                string q = "select * from tbl_Rawprod where RCode='" + txtcode.Text + "'";
                DataTable purchase = clsDataLayer.RetreiveQuery(q);
                if (purchase.Rows.Count > 0)
                {
                    rptRawProduct pr = new rptRawProduct();
