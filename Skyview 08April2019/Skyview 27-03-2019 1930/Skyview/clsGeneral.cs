...
namespace Skyview
{
    public static class clsGeneral
    {
        public static string getMAXCode(string tableName, string columnName, string format)
        {
            ...
            DataTable dt = clsDataLayer.RetreiveQuery(" SELECT ISNULL( MAX(" + columnName + "),0) AS MAXID FROM " + tableName);
        ...
        {
            DataTable dt = clsDataLayer.RetreiveQuery(" SELECT " + ValueMember + " , " + DisplayMember + " FROM " + tableName);
